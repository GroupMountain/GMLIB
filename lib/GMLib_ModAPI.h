...
enum class AllExperiments {
    ...
    Cameras = 12,                    // 实验相机
    VillagerTradesRebalance = 15,    // 村民贸易再平衡
};
...
struct BiomeData {
    BiomeType type;
    ClimateUtils::Parameter temperatureRange, humidityRange, continentalnessRange, erosionRange, weirdnessRange;
    float offset;
};
...
class GMLib_Mod {
   public:
    ...
    // 新增一个资源包和行为包的加载路径
    GMLib_ModAPI static void addAddonsFilePath(std::string path);

    // 实验性玩法API
    // 开关实验性（请在'ServerStartedEvent'之后再使用，如非需要关闭实验性，请尽量不要使用此函数）。
    // 不建议直接使用此函数，请使用'addExperimentsRequire'直接添加所需实验性依赖，GMLIB会自动启用需要的实验性。
    ...
    // 添加实验性玩法依赖（注册所需实验性依赖，GMLIB会自动开启对应的实验性）
    ...
    //熔炉/营火合成表，tags可填{"furnace", "blast_furnace", "smoker", "campfire", "soul_campfire"}
    ...
    //删除合成表
    GMLib_ModAPI static bool removeRecipe(std::string recipe_id);
    //更新客户端合成表（玩家在线的时候修改了合成表才需要更新）
    GMLib_ModAPI static void updateClientRecipes();
    ...
    // 自定义生物群系（以下API会自动添加实验性依赖，无需手动添加）
    // 设置生物群系气候（注册此项之后，自定义生物群系才会生成）
    GMLib_ModAPI static void registerBiomeClimates(std::string id, BiomeData data);
    // 注册空白生物群系，不建议使用
    GMLib_ModAPI static void registerEmptyBiome(std::string id, BiomeData data);
};