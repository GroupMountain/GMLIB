...
class ItemStack : public ::ItemStack {
public:
    GMLIB_NDAPI ItemStack();

    GMLIB_NDAPI explicit ItemStack(class ItemInstance const& item);

    GMLIB_NDAPI ItemStack(class ::ItemStack const& item);

    GMLIB_NDAPI explicit ItemStack(class RecipeIngredient const& ingredient);

    GMLIB_NDAPI ItemStack(class BlockLegacy const& blockLegacy, int count = 1);

    GMLIB_NDAPI ItemStack(class Block const& block, int count = 1, class CompoundTag const* userData = nullptr);
