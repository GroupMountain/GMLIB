...
public:
    GMLIB_NDAPI static optional_ref<Level> getInstance();

    GMLIB_NDAPI static optional_ref<Level> getLevel();
    ...
    GMLIB_API static bool saveLevelDat(::CompoundTag const& data);
...
public:
    ...
    GMLIB_API void sendPacketRawToClients(Packet& packet);

    GMLIB_API void sendPacketRawToDimension(Packet& packet, DimensionType dimId);

    GMLIB_API void sendPacketRawTo(Packet& packet, Player& player);

    GMLIB_API void sendPacketRawTo(Packet& packet, NetworkIdentifier const& netId);
