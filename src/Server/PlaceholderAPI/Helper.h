...
inline std::tuple<bool, std::unordered_map<std::string, std::string>>
FindPlaceholder(std::string str, std::string str2) {
    ...
    if (map.empty() || !isSame(ori, input, ori.size())) return std::make_tuple(false, map);
    else return std::make_tuple(true, map);
}
